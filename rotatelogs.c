...
int main (int argc, const char * const argv[])
{
    ...
    for (;;) {
        ...
        if (tRotation) {
            ...
            }  else if (nLogFD != NULL && !isToday(&curTime)) {
				// 여기에서 현재의 날짜를 시간으로 저장하는 것이 중요하다.
				// now와 tLogEnd와 비교를 해야 한다.
				//                 여기서 비교란 날짜를 비교하는 것이다. 
				nLogFDprev = nLogFD;
		        nLogFD = NULL;
				setCurrentTime(&curTime);
		    }
...
int isToday(CurTime *cTime) {
	...
	apr_time_exp_lt (&exp, currentTime);
	return ((cTime->year == exp.tm_year) && (cTime->month == exp.tm_mon) && (cTime->day == exp.tm_mday));
}

int setCurrentTime(CurTime *cTime) {
	apr_time_exp_t lt;
        apr_time_exp_lt(&lt, apr_time_now());
        cTime->year = lt.tm_year;
        cTime->month = lt.tm_mon;
        cTime->day = lt.tm_mday;
}
